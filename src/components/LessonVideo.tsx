  ...
  return (
    <>
      ...
      <div className="relative w-full aspect-video rounded-2xl overflow-hidden bg-gradient-to-br from-slate-900 via-slate-950 to-blue-950 text-white shadow-2xl mb-6">
        ...
        {started && (
          <div className="absolute bottom-0 left-0 right-0 p-3 md:p-4 bg-gradient-to-t from-black/70 to-transparent flex items-center gap-2 md:gap-3">
            ...
            <div className="text-xs font-mono text-white/70 tabular-nums">
              {Math.floor(elapsedRef.current / 1000)}s / {Math.floor(TOTAL_DURATION / 1000)}s
            </div>
          </div>
        )}
        ...
        {!started && (
          <div
            className="absolute bottom-0 left-0 h-1 bg-gradient-to-r from-sky-400 to-amber-400"
            style={{ width: `${progress * 100}%` }}
          />
        )}
      </div>
